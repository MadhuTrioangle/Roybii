...
class CustomTextFormField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding:
          EdgeInsets.only(left: (padLeft ?? 10.0), right: (padRight ?? 10)),
      child: TextFormField(
        ...
        decoration:InputDecoration(
          ...
          border:isBorder == true ? InputBorder.none : OutlineInputBorder(
            borderRadius: BorderRadius.circular(size),
            borderSide: const BorderSide(
              width: 1.0,
            ),
          ),
          errorBorder:isBorder == true ? InputBorder.none :  OutlineInputBorder(
            borderRadius: BorderRadius.circular(size),
            borderSide: BorderSide(color: red, width: 1.0),
          ),
          enabledBorder:isBorder == true ? InputBorder.none :  OutlineInputBorder(
            borderRadius: BorderRadius.circular(size),
            borderSide: BorderSide(
              color: themeSupport().isSelectedDarkMode() ? white : enableBorder,
              width: 1.0,
            ),
          ),
          focusedBorder:isBorder == true ? InputBorder.none :  OutlineInputBorder(
            borderRadius: BorderRadius.circular(size),
            borderSide: BorderSide(
              color: themeColor,
              width: 1.0,
          ...
          hintText: text, errorStyle: TextStyle(color: red), errorMaxLines: 2,
          hintStyle: TextStyle(
