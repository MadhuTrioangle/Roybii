...
import 'package:zodeakx_mobile/Common/Wallets/CryptoDeposit/View/selectWalletModal.dart';
...
class _CryptoDepositViewState extends State<CryptoDepositView> {
  ...
  Widget network(){
    return Column(
      ...
      children: [
        CustomText(text: stringVariables.network,color: stackCardText,fontsize: 16,fontWeight: FontWeight.w500,),
        CustomSizedBox(height: 0.01,),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            CustomText(text: getCurrencyViewModel.network.toString(),fontsize: 14,fontWeight: FontWeight.w600,),
            GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: (){
                showNetworkModel();
              },
                child: SvgPicture.asset(exchangeMargin,color: stackCardText,height: 15,))
          ],
        ),
        getCurrencyViewModel.network != "XRP" ? CustomSizedBox() :
        Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            CustomSizedBox(height: 0.02,),
            CustomText(text: stringVariables.tagID,color: stackCardText,fontsize: 16,fontWeight: FontWeight.w500,),
            CustomSizedBox(height: 0.015,),
            CustomText(text:context.watch<CoinDetailsViewModel>().findAddress?.tagId ?? "Loading...",fontsize: 14,fontWeight: FontWeight.w600,),
            CustomSizedBox(height: 0.01,),
          ],
        ),
        CustomSizedBox(height: 0.01,),
        CustomText(text: stringVariables.walletAddress,color: stackCardText,fontsize: 16,fontWeight: FontWeight.w500,),
        CustomSizedBox(height: 0.001,),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            CustomContainer(
              width: 1.6,
                child: CustomText(text: context.watch<CoinDetailsViewModel>().findAddress?.address ?? "Loading...",fontsize: 13.5,fontWeight: FontWeight.w500,)),
            CustomIconButton(
              onPress: () {
        ...
        Row(
          children: [
            CustomText(text: stringVariables.selectWallet,color: stackCardText,fontsize: 16,fontWeight: FontWeight.w500,),
            CustomSizedBox(
              width: 0.02,
            ),
            GestureDetector(
              onTap: (){
                ...
                child: Icon(Icons.info,color: textGrey,size: 18,))
          ],
        ),
        CustomSizedBox(height: 0.01,),
        CustomText(text: viewModel.wallet,fontsize: 14,fontWeight: FontWeight.w600,)
      ],
    );
  }
  ...
  _showModal() async {
    final result =
        await Navigator.of(context).push(SelectWalletModal(context));
  }
  ...
  Widget qrCode() {
    ...
    return Column(
      children: [
        ...
        CustomText(text: '${stringVariables.sendOnly} ${constant.walletCurrency.value}' ' ${stringVariables.toDepositAddress}',color: stackCardText,fontsize: 13,),
        CustomSizedBox(
          height: 0.03,
        ),
        Divider()
  ...
  // }
   ...
   showNetworkModel() async {
     final result = await Navigator.of(context).push(SelectWalletBottomSheet(context,1));
   }
