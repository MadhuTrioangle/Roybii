...
import 'GetcurrencyViewModel.dart';
...
class CoinDetailsViewModel extends ChangeNotifier {
    ...
    if (hasInternet) {
      ...
      switch (decodeResponse.status?.index) {
        case 0:
          ...
          break;
        case 1:
          setInOrderBalance(decodeResponse.data?.result);
          break;
        default:
          setLoading(false);
          break;
  ...
  /// Find Address
  getFindAddress(String? currencyCode,String? ntwk) async {
    ...
    var hasInternet = await checkInternet.hasInternet();
    String network = '';
    if (currencyCode!.isEmpty && ntwk!.isNotEmpty) {
      network = ntwk;
    } else {
      getCurrency?.forEach((element) {
        if (element.currencyCode == currencyCode) {
          network = element.network!.first;
        }
      });
    }
    securedPrint("network${network}");
    if (hasInternet) {
      String? query;
      switch (network) {
        case "ETH":
